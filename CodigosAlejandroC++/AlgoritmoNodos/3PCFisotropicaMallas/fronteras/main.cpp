...
void save_histogram(string name, int bns, unsigned int ***histo){
	int i,j,k;
	int bbns = bns*bns;
	/*Se crea un arreglo auxiliar para redimensionar el histograma a una matriz de dimensiones [bns][bns*bns]*/
	unsigned int **reshaped = new unsigned int*[bns];
	for (i = 0; i < bns; i++)
	{
		*(reshaped + i) = new unsigned int[bns*bns];
	}
	//Redimensionando el histograma
	for ( i = 0; i < bns; i++)
	{
		for ( j = 0; j < bns; j++)
		{
			for ( k = 0; k < bns; k++)
			{
				reshaped[i][bns*j + k] = histo[i][j][k];	
			}	
		}	
	}
